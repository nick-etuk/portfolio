...
for db in db_list:
    ...
    try:
        ...
        # Dump database
        p = Popen(["mysqldump", "--login-path=" + db["loginPath"],
                  db["dbName"]], stdout=PIPE, stderr=PIPE)
        out, err = p.communicate()
        if len(err) > 0:
            raise Exception(err)
        f = open(pipes.quote(today_bkp_path) +
                 "/" + db["dbName"] + ".sql", "w")
        f.write(out)
        f.close()
        ...
        os.system(compcmd)
        delcmd = "rm " + pipes.quote(today_bkp_path) + \
            "/" + db["dbName"] + ".sql"
        os.system(delcmd)

        print("Backup script completed")
        print("Your backups has been created in '" +
              today_bkp_path + "' directory\n")
